...
<a href="{{ route('admin.sales_matrial_types.index') }}">  الوحدات </a>
@endsection
      ...
      <div class="form-group">
        ...
        <div class="form-group"> 
          ...
          <select name="is_master" id="is_master" class="form-control">
           ...
           <option {{  old('active',$data['is_master'])==1 ? 'selected' : ''}}   value="1"> وحدة اب </option>
           <option {{ old('active',$data['is_master'])==0 ? 'selected' : ''}}  value="0"> تجزئة</option>
          </select>
          @error('is_master')
          <span class="text-danger">{{ $message }}</span>
          @enderror
